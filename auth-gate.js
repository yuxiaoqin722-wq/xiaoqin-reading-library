...
(function () {
  ...
  function inject() {
    ...
    ov.innerHTML =
      ...
      '<input type="email" id="agEmail" placeholder="you@example.com">' +
      '</div>' +
      '<div class="ag-field">' +
      '<label>密码 Password</label>' +
      '<input type="password" id="agPass" placeholder="密码 Password">' +
      '</div>' +
    ...
    gr.id = 'agGreet';
    gr.innerHTML =
      '<span class="g-name" id="agGName"></span>' +
      '<button class="g-out" id="agGOut" type="button">退出 Sign out</button>';
  ...
  function showGate() {
    ...
    ov.classList.remove('ag-hide');
    q('agGreet').style.display = 'none';
  }

  function hideGate(u) {
    document.documentElement.style.overflow = '';
    ov.classList.add('ag-hide');

    if (u) {
      q('agGName').textContent = 'Hi ' + nameOf(u);
      q('agGreet').style.display = 'flex';
    }
  }

  function getHaoteachSSOTokens() {
    try {
      ...
      var ht_at = params.get('ht_at');
      var ht_rt = params.get('ht_rt');

      if (!ht_at || !ht_rt) return null;
  ...
  async function submit() {
    ...
    var pass = q('agPass').value;
    var name = q('agName').value.trim();
    var err = q('agErr');
    ...
    if (!email || !pass) {
      err.textContent = '请输入邮箱和密码 Enter email & password';
      return;
    }
    ...
    try {
      if (tab === 'signup') {
        if (pass.length < 6) {
          err.textContent = '密码至少6位 Min 6 chars';
          btn.disabled = false;
          return;
        }
        ...
        if (r.data && r.data.session) {
          hideGate(r.data.user);
          return;
        }

        err.style.color = '#1a6637';
        err.textContent = '✅ 注册成功,请去邮箱确认后登录';
        setTab('signin');
      } else {
        var r2 = await _t(
        ...
        if (r2.error) {
          err.textContent = '邮箱或密码错误 Incorrect email or password';
          btn.disabled = false;
          return;
        }
      ...
      err.textContent = '请求超时,请重试 Timed out, try again';
    }

    btn.disabled = false;
  }
    function wire() {
    q('agTabIn').onclick = function () {
      setTab('signin');
    };

    q('agTabUp').onclick = function () {
      setTab('signup');
    };

    q('agSubmit').onclick = submit;

    q('agPass').addEventListener('keydown', function (e) {
      if (e.key === 'Enter') submit();
    });

    q('agGOut').onclick = async function () {
      try {
        await _t(sb.auth.signOut(), 6000);
      } catch (e) {}
